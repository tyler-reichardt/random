...
import numpy as np
import pandas as pd
from pyspark.sql.functions import col, regexp_replace, max, expr, row_number, when, datediff, rank, array_contains, array
...
df.write.format("delta").option("overwriteSchema", "true").mode("overwrite").saveAsTable(raw_table_name)
