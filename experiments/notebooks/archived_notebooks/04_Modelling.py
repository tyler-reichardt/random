...
other_cols = ['claim_number','svi_risk', 'dataset', 'fa_risk']
...
raw_df = raw_df.withColumn('svi_risk', col('fa_risk').cast('int')).fillna({'svi_risk': 0})
#raw_df = raw_df.withColumn('svi_risk', when(col('svi_risk').isin([0, 1]), 1).otherwise(0))
...
raw_df_pd = raw_df.drop(*high_cardinality).toPandas().sort_values(by='claim_number', ascending=False).drop(['claim_number', 'fa_risk'], axis=1)
...
with mlflow.start_run():
    ...
    param_grid = {
        'classifier__n_estimators': [20, 50, 70],
        'classifier__max_depth': [4, 5, 6],
        'classifier__learning_rate': [0.01],
        'classifier__num_leaves': [8, 16, 32, 64],
