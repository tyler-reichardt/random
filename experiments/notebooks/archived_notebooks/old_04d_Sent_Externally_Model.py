...
other_cols = ['claim_number','svi_risk', 'dataset', 'fa_risk', 'tbg_risk']
...
categorical_transformer = Pipeline(steps=[
    ('onehot', OneHotEncoder(handle_unknown='ignore', drop='if_binary', max_categories=5))
])
...
# Define the model
lg_model = LGBMClassifier(verbose=0, max_depth=4, num_leaves=20, n_estimators=70)

X_prep = preprocessor.fit_transform(X_train)
X_prep_test = preprocessor.transform(X_test)

lg_model = lg_model.fit(X_prep, y_train)

display(simple_classification_report(lg_model, X_prep, y_train, threshold=0.5))
display(simple_classification_report(lg_model, X_prep_test, y_test, threshold=0.5))
'''
...
mdi_importances = pd.Series(
    lg_model.feature_importances_, index=feature_names
...
with mlflow.start_run():
    ...
    mlflow.set_tag("model_name", "svi_fa_risk")
    # Identify numeric and categorical columns
    numeric_features = X_train.select_dtypes(include =['number']).columns
    categorical_features = X_train.select_dtypes(include=['object', 'category']).columns
    print("numeric: ", numeric_features)
    ...
    xgb_model = LGBMClassifier(verbose=0, scale_pos_weight=10)
    ...
    param_grid = {
        ...
        'classifier__num_leaves': [5, 10, 15, 31, 64],
    ...
    #    param_distributions=param_grid, cv=3, scoring="recall", verbose=0, n_iter=20, random_state=42)

    # Set up GridSearchCV
    grid_search = GridSearchCV(estimator=pipeline, param_grid=param_grid, cv=3, scoring='f1', verbose=0)
    # Fit the model
    ...
    #add the true and predicted targets to test set
    ...
    #recall=scoring, scale_pos=100

    '''
    #'classifier__n_estimators': [20, 50, 70, 100],
        'classifier__max_depth': [4, 5, 6],
        'classifier__learning_rate': [0.01],
        'classifier__num_leaves': [7, 15, 31, 63, 127],
        'classifier__min_child_samples': [8, 16, 20, 32, 64]
    '''
...
print(lgb_classifier.get_params())
print(lg_model.get_params())
...
print(pd.DataFrame(train_predictions).value_counts())
