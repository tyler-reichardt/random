...
run_id = "d3d4f63149fd4f7584a6538b0bf13fb7"
...
numeric2= ['policyholder_ncd_years', 'inception_to_claim', 'min_claim_driver_age', 'veh_age', 'business_mileage', 'annual_mileage', 'incidentHourC', 'additional_vehicles_owned_1', 'age_at_policy_start_date_1', 'cars_in_household_1', 'licence_length_years_1', 'years_resident_in_uk_1', 'max_additional_vehicles_owned', 'min_additional_vehicles_owned', 'max_age_at_policy_start_date', 'min_age_at_policy_start_date', 'max_cars_in_household', 'min_cars_in_household', 'max_licence_length_years', 'min_licence_length_years', 'max_years_resident_in_uk', 'min_years_resident_in_uk', 'impact_speed', 'voluntary_amount', 'vehicle_value', 'manufacture_yr_claim', 'outstanding_finance_amount', 'claim_to_policy_end', 'incidentDayOfWeekC', 'areasDamagedMinimal', 'areasDamagedMedium', 'areasDamagedHeavy', 'areasDamagedSevere', 'areasDamagedTotal', ] #+ ['fa_pred'] #'damageScore', 

categorical_features = ['vehicle_unattended', 'excesses_applied', 'is_first_party', 'first_party_confirmed_tp_notified_claim', 'is_air_ambulance_attendance', 'is_ambulance_attendance', 'is_fire_service_attendance', 'is_police_attendance', 'veh_age_more_than_10', 'police_considering_actions', 'is_crime_reference_provided', 'ncd_protected_flag', 'boot_opens', 'doors_open', 'multiple_parties_involved', 'is_incident_weekend', 'is_reported_monday', 'driver_age_low_1', 'claim_driver_age_low', 'licence_low_1', 'C1_fri_sat_night', 'C2_reporting_delay', 'C3_weekend_incident_reported_monday', 'C5_is_night_incident', 'C6_no_commuting_but_rush_hour', 'C7_police_attended_or_crime_reference', 'C9_policy_within_30_days', 'C10_claim_to_policy_end', 'C11_young_or_inexperienced', 'C12_expensive_for_driver_age', 'C14_contains_watchwords', 'vehicle_overnight_location_id', 'policy_type', 'assessment_category', 'engine_damage', 'sales_channel', 'overnight_location_abi_code', 'vehicle_overnight_location_name', 'policy_cover_type', 'notification_method', 'impact_speed_range', 'incident_type', 'incident_cause', 'incident_sub_cause', 'front_severity', 'front_bonnet_severity', 'front_left_severity', 'front_right_severity', 'left_severity', 'left_back_seat_severity', 'left_front_wheel_severity', 'left_mirror_severity', 'left_rear_wheel_severity', 'left_underside_severity', 'rear_severity', 'rear_left_severity', 'rear_right_severity', 'right_severity', 'right_back_seat_severity', 'right_front_wheel_severity', 'right_mirror_severity', 'right_rear_wheel_severity', 'right_roof_severity', 'right_underside_severity', 'roof_damage_severity', 'underbody_damage_severity', 'windscreen_damage_severity', 'incident_day_of_week', 'reported_day_of_week', 'checks_max']

#'impact_speed_unit', 'rear_window_damage_severity', 'windscreen_damage_severity', 'incidentMonthC',
...
interview_features = list(set(numeric2) | set(categorical_features)- set(['additional_vehicles_owned_1', 'max_additional_vehicles_owned', 'min_additional_vehicles_owned', 'cars_in_household_1', 'max_cars_in_household', 'min_cars_in_household'])) 

num_interview = [ 'policyholder_ncd_years', 'min_claim_driver_age', 'max_licence_length_years', 'min_age_at_policy_start_date', 'age_at_policy_start_date_1', 'min_years_resident_in_uk', 'max_years_resident_in_uk', 'claim_to_policy_end', 'min_additional_vehicles_owned', 'annual_mileage', 'incidentDayOfWeekC', 'areasDamagedHeavy', 'additional_vehicles_owned_1', 'areasDamagedSevere', 'areasDamagedTotal', 'inception_to_claim',  'voluntary_amount', 'min_cars_in_household', 'incidentHourC', 'vehicle_value', 'veh_age']

cat_interview = ['C5_is_night_incident','C7_police_attended_or_crime_reference', 'claim_driver_age_low', 'driver_age_low_1', 'is_ambulance_attendance', 'licence_low_1', 'assessment_category', 'C11_young_or_inexperienced', 'C2_reporting_delay', 'front_bonnet_severity', 'incident_cause', 'C12_expensive_for_driver_age', 'engine_damage', 'ncd_protected_flag', 'impact_speed_range', 'front_left_severity', 'first_party_confirmed_tp_notified_claim', 'C9_policy_within_30_days', 'C14_contains_watchwords', 'front_severity'] #'is_air_ambulance_attendance',

X_train = X_train[num_interview + cat_interview + ['fa_pred']]
X_test = X_test[num_interview + cat_interview + ['fa_pred']]
...
print([x for x in X_train.columns if '_household' in x])
...
with mlflow.start_run():
    ...
    mlflow.set_tag("model_name", "svi_interview_simple")
    ...
    xgb_model = LGBMClassifier(verbose=-1)
    ...
    param_grid = {
        'classifier__n_estimators': [32,64],
        'classifier__max_depth': [4, 5, 6],
        'classifier__learning_rate': [0.01],
        'classifier__num_leaves': [5, 15, 30, 50, 100],
        'classifier__min_child_samples': [4, 8, 16, 32]
    }

    #param_grid = { 'classifier__C': [0.01, 0.1, 1, 10, 100], 'classifier__penalty': ['l1', 'l2'], 'classifier__solver': ['liblinear', 'saga'] }
    ...
    # Set up GridSearchCV
    grid_search = GridSearchCV(estimator=pipeline, param_grid=param_grid, cv=3, scoring='roc_auc', verbose=0)
    # Fit the model
    ...
    display(metrics_df) 
    mlflow.log_dict(metrics_df.to_json(orient='split'), "metrics_df")
...
#with fa_pred: https://adb-6646029803360125.5.azuredatabricks.net/ml/experiments/6d0ed0100f0f4b7791f9e6883c45e2f1/runs/95235c6f1cec4284a9792b010151b2d9
# no fa pred: https://adb-6646029803360125.5.azuredatabricks.net/ml/experiments/6d0ed0100f0f4b7791f9e6883c45e2f1/runs/98a9442bfa6645dc9da10f9c0f50e25a
# log_reg = https://adb-6646029803360125.5.azuredatabricks.net/ml/experiments/6d0ed0100f0f4b7791f9e6883c45e2f1/runs/5d214d51b5734402b503d1661ab51a74
...
# COMMAND ----------
 ...
 basic_pipeline= grid_search.best_estimator_
feature_names = basic_pipeline.named_steps['preprocessor'].get_feature_names_out()
importances = basic_pipeline.named_steps['classifier'].feature_importances_
...
plt.hist(pd.DataFrame(basic_pipeline.predict_proba(X_train)))

print( basic_pipeline.named_steps['classifier'].get_params( ))
...
# Define the model
#xgb_model = xgb.XGBClassifier(objective='binary:logistic')
xgb_model = LGBMClassifier(verbose=0, n_estimators=20, )
# Create a pipeline that combines preprocessing and the model
...
basic_pipeline = basic_pipeline.fit(X_train, y_train)

metrics_train = simple_classification_report(basic_pipeline, X_train, y_train)
metrics_test = simple_classification_report(basic_pipeline, X_test, y_test)
metrics_train_dt = simple_classification_report(dt_pipeline, X_train, y_train)
metrics_test_dt = simple_classification_report(dt_pipeline, X_test, y_test)
metrics_df = pd.DataFrame([metrics_train, metrics_test, metrics_train_dt, metrics_test_dt], index=['train_gb', 'test_gb', 'train_dt', 'test_dt']).round(3).reset_index()
display(metrics_df) 
...
# Assuming 'basic_pipeline.named_steps['classifier']' is a DecisionTreeClassifier
classifier = basic_pipeline.named_steps['classifier']

# Plot the decision tree
fig = plt.figure(figsize=(140,50))
_ = plot_tree(classifier,
                   feature_names=feature_names.tolist(), fontsize = 20,
                   filled=True)

# COMMAND ----------

y_test_df = pd.DataFrame(y_test.values, columns=['y_test'])
y_test_df['y_pred'] = pd.DataFrame(basic_pipeline.predict(X_test))
display(y_test_df)
...
for column in categorical_features:
    ...
    if (test_categories - train_categories):
        ...
        print("Categories in X_test but not in X_train:", test_categories - train_categories)
        print("\n")
...
correlation_matrix = pd.concat([X_train, y_train_df], axis=1).corr(numeric_only=True)
#correlation_matrix = X_train[numeric2+[target_variable]].corr(numeric_only=True)
...
display( pd.concat([X_train, y_train_df]))

num_interview = [ 'policyholder_ncd_years', 'min_claim_driver_age', 'max_licence_length_years', 'min_age_at_policy_start_date', 'age_at_policy_start_date_1', 'min_years_resident_in_uk', 'max_years_resident_in_uk', 'claim_to_policy_end', 'min_additional_vehicles_owned', 'annual_mileage', 'incidentDayOfWeekC', 'areasDamagedHeavy', 'additional_vehicles_owned_1', 'areasDamagedSevere', 'areasDamagedTotal', 'inception_to_claim',  'voluntary_amount', 'min_cars_in_household', 'incidentHourC', 'vehicle_value', 'veh_age']

cat_interview = ['C5_is_night_incident','C7_police_attended_or_crime_reference', 'claim_driver_age_low', 'driver_age_low_1', 'is_air_ambulance_attendance', 'is_ambulance_attendance', 'licence_low_1', 'assessment_category', 'C11_young_or_inexperienced', 'C2_reporting_delay', 'front_bonnet_severity', 'incident_cause', 'C12_expensive_for_driver_age', 'engine_damage', 'ncd_protected_flag', 'impact_speed_range', 'front_left_severity', 'first_party_confirmed_tp_notified_claim', 'C9_policy_within_30_days', 'C14_contains_watchwords', 'front_severity']
...
encoded_X_train = X_train[categorical_features].copy()
for column in categorical_features:
    encoded_X_train[column] = label_encoder.fit_transform(encoded_X_train[column])

# Apply Chi-Square test
chi_scores, p_values = chi2(encoded_X_train, y_train_encoded)
...
chi2_results['signif'] = chi2_results['P-Value'] <= 0.05

# Display the results sorted by p-value
display(chi2_results.sort_values('P-Value').reset_index(drop=True))

display( encoded_X_train.min().sort_values())
