...
class VehicleAgeTransformer(BaseEstimator, TransformerMixin):
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        if 'fp_vehicle_age' not in output_features:
            output_features.append('fp_vehicle_age')
        return np.array(output_features)
...
def standardize_pandas_schema(df: pd.DataFrame) -> pd.DataFrame:
    ...
    missing_value_strings = [
        'none', 'null', 'nan', '', ' ',
        'na', 'n/a', 'not available', 'unknown', '-',
        '#n/a', '#n/a n/a', 'n/a (not applicable)', 'unspecified'
    ]
...
# ─── Base Transformers (1-14 remain the same) ─────────────────────────────────

class SplatmapExtractor(BaseEstimator, TransformerMixin):
    def __init__(self, splatmap_cols=None):
        if splatmap_cols is None:
            splatmap_cols = [
                "Front", "FrontBonnet", "FrontLeft", "FrontRight", "Left",
                "LeftBackseat", "LeftFrontWheel", "LeftMirror", "LeftRearWheel",
                "LeftRoof", "LeftUnderside", "LoadArea", "Rear", "RearLeft",
                "RearRight", "RearWindowDamage", "Right", "RightBackseat",
                "RightFrontWheel", "RightMirror", "RightRearWheel", "RightRoof",
                "RightUnderside", "RoofDamage", "UnderbodyDamage", "WindscreenDamage"
            ]
        self.splatmap_cols = splatmap_cols
...
class RenameColumnsTransformer(BaseEstimator, TransformerMixin):
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        output_features = []
        for feature in input_features:
            output_features.append(self.rename_map.get(feature, feature))
        return np.array(output_features)
...
class ImpactSpeedTransformer(BaseEstimator, TransformerMixin):
    ...
    def get_feature_names_out(self, input_features=None):
        if input_features is None:
            raise ValueError("input_features must be provided")
        output_features = [f for f in input_features if f not in ['impact_speed_range', 'impact_speed_unit']]
        if 'impact_speed' not in output_features:
            output_features.append('impact_speed')
        return np.array(output_features)
...
class CarCharacteristicTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        yes_no_cols = [
            'fp_boot_opens', 'fp_doors_open', 'fp_wheels_damaged', 'fp_lights_damaged',
            'tp_engine_capacity', 'tp_number_of_doors', 'tp_boot_opens', 'tp_doors_open',
            'tp_not_on_mid', 'tp_vehicle_unattended', 'tp_wheels_damaged', 'tp_lights_damaged'
        ]
        ...
        # Create damage assessment dummies
        if 'tp_driveable_damage_assessment' in df.columns:
            df['da_dr'] = (df['tp_driveable_damage_assessment'] == 'DriveableRepair').astype(int)
            df['da_dtl'] = (df['tp_driveable_damage_assessment'] == 'DriveableTotalLoss').astype(int)
            df['da_utl'] = (df['tp_driveable_damage_assessment'] == 'UnroadworthyTotalLoss').astype(int)
            df['da_ur'] = (df['tp_driveable_damage_assessment'] == 'UnroadworthyRepair').astype(int)
            df['da_o'] = ~df['tp_driveable_damage_assessment'].isin([
                'DriveableRepair', 'DriveableTotalLoss',
                'UnroadworthyTotalLoss', 'UnroadworthyRepair'
            ]).astype(int)
            df = df.drop(['tp_driveable_damage_assessment'], axis=1)

        # Handle FP damage assessment if it exists
        if 'fp_driveable_damage_assessment' in df.columns:
            # You might want to create similar dummies for FP or just drop it
            df = df.drop(['fp_driveable_damage_assessment'], axis=1)
    ...
    def get_feature_names_out(self, input_features=None):
        ...
        cols_to_remove = ['tp_driveable_damage_assessment', 'fp_driveable_damage_assessment']
        output_features = [f for f in output_features if f not in cols_to_remove]
...
class VehicleAgeTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # Create both FP and TP vehicle age if year_of_manufacture exists
        if 'year_of_manufacture' in df.columns and 'incident_date' in df.columns:
            df['fp_vehicle_age'] = df['incident_date'].dt.year - df['year_of_manufacture'].astype(int)
            df['fp_vehicle_age'] = df['fp_vehicle_age'].where(df['fp_vehicle_age'] <= 30, 0).fillna(0)
           
            # Also create tp_vehicle_age if it doesn't exist
            df['tp_vehicle_age'] = df['fp_vehicle_age']  # Assuming same as FP for now
...
class RHDTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        df = X.copy()
        if 'tp_right_hand_drive' in df.columns:
            df['tp_right_hand_drive'] = (df['tp_right_hand_drive'] == 'R').astype(int)
        return df
...
class BodyKeyEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        df = X.copy()
        for i, style in enumerate(self.keys, start=1):
            if 'fp_body_key' in df.columns:
                df[f'fp_body_key_0{i}'] = (df['fp_body_key'] == style).astype(int)
            if 'tp_body_key' in df.columns:
                df[f'tp_body_key_0{i}'] = (df['tp_body_key'] == style).astype(int)
       
        cols_to_drop = []
        if 'fp_body_key' in df.columns:
            cols_to_drop.append('fp_body_key')
        if 'tp_body_key' in df.columns:
            cols_to_drop.append('tp_body_key')
...
class PostcodeAreaExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if 'postcode_area' in df.columns:
            df['postcode_area'] = df['postcode_area'].str.extract(r'^([A-Za-z]+)')[0].fillna('ZZ')
        return df
...
class DamageTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, severity_columns=None, fp_severity_columns=None):
        self.severity_columns = severity_columns or [
            'tp_front_severity', 'tp_front_bonnet_severity', 'tp_front_left_severity',
            'tp_front_right_severity', 'tp_left_severity', 'tp_left_back_seat_severity',
            'tp_left_front_wheel_severity', 'tp_left_mirror_severity', 'tp_left_rear_wheel_severity',
            'tp_left_roof_severity', 'tp_left_underside_severity', 'tp_load_area_severity',
            'tp_rear_severity', 'tp_rear_left_severity', 'tp_rear_right_severity',
            'tp_rear_window_damage_severity', 'tp_right_severity', 'tp_right_back_seat_severity',
            'tp_right_front_wheel_severity', 'tp_right_mirror_severity', 'tp_right_rear_wheel_severity',
            'tp_right_roof_severity', 'tp_right_underside_severity', 'tp_roof_damage_severity',
            'tp_underbody_damage_severity', 'tp_windscreen_damage_severity'
        ]

        self.fp_severity_columns = fp_severity_columns or [
            'fp_front_severity', 'fp_front_bonnet_severity', 'fp_front_left_severity',
            'fp_front_right_severity', 'fp_left_severity', 'fp_left_back_seat_severity',
            'fp_left_front_wheel_severity', 'fp_left_mirror_severity', 'fp_left_rear_wheel_severity',
            'fp_left_roof_severity', 'fp_left_underside_severity', 'fp_load_area_severity',
            'fp_rear_severity', 'fp_rear_left_severity', 'fp_rear_right_severity',
            'fp_rear_window_damage_severity', 'fp_right_severity', 'fp_right_back_seat_severity',
            'fp_right_front_wheel_severity', 'fp_right_mirror_severity', 'fp_right_rear_wheel_severity',
            'fp_right_underside_severity', 'fp_roof_damage_severity', 'fp_underbody_damage_severity',
            'fp_windscreen_damage_severity', 'fp_panel_gaps', 'fp_radiator_damaged', 'fp_sharp_edges'
        ]
    ...
    def transform(self, X):
        ...
        existing_sev_cols = [col for col in self.severity_columns if col in df.columns]
        existing_fp_cols = [col for col in self.fp_severity_columns if col in df.columns]
       
        if existing_sev_cols:
            df['damage_recorded'] = df[existing_sev_cols].notnull().sum(axis=1)
            df['tp_damage_assessed'] = df['damage_recorded'].apply(lambda x: 1 if x > 0 else 0)
        else:
            df['damage_recorded'] = 0
            df['tp_damage_assessed'] = 0
       
        def damage_scale(row_area, row_damageflag):
            if row_damageflag == 1:
                if row_area == 'minimal': return 1
                elif row_area == 'medium': return 2
                elif row_area == 'heavy': return 3
                elif row_area == 'severe': return 4
                elif row_area == 'unknown': return -1
                else: return 0
            else:
                return -1

        for col_name in existing_sev_cols:
            df[col_name] = df.apply(lambda row: damage_scale(row[col_name], row['tp_damage_assessed']), axis=1).astype(int)
       
        for col_name in existing_fp_cols:
...
class DamageSeverityCalculator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        existing_cols = [col for col in self.sev_damage if col in df.columns]
       
        if existing_cols:
            df['damage_sev_total'] = df[existing_cols].sum(axis=1)
            df['damage_sev_count'] = df[existing_cols].gt(0).sum(axis=1)
            df['damage_sev_mean'] = df.apply(
                lambda row: row['damage_sev_total'] / row['damage_sev_count'] if row['damage_sev_count'] > 0 else 0,
                axis=1
            )
            df['damage_sev_max'] = df[existing_cols].max(axis=1)
        else:
            df['damage_sev_total'] = 0
            df['damage_sev_count'] = 0
            df['damage_sev_mean'] = 0
            df['damage_sev_max'] = 0
...
# ─── REFACTORED: Days In Repair Predictor with Preprocessing ─────────────────

class DaysInRepairPredictorWithPreprocessing(BaseEstimator, TransformerMixin):
    """
    Enhanced Days In Repair Predictor that includes its own preprocessing pipeline
    to ensure all required features are created before prediction.
    """
    def __init__(self, model_uri, preprocessor_pipeline=None):
        self.model_uri = model_uri
        self.preprocessor_pipeline = preprocessor_pipeline
        self.model = None
        self.input_cols_from_schema = None
        self.model_schema = None
        self.col_to_type_map = None
    ...
    def fit(self, X, y=None):
        ...
        # Fit preprocessor if provided
        if self.preprocessor_pipeline is not None:
            self.preprocessor_pipeline.fit(X)
    ...
    def transform(self, X):
        ...
        # Apply preprocessing if pipeline is provided
        if self.preprocessor_pipeline is not None:
            X_processed = self.preprocessor_pipeline.transform(X_original)
        else:
            X_processed = X_original
           
        # Prepare data for model
        X_for_model = pd.DataFrame(index=X_processed.index)
       
        for col_name in self.input_cols_from_schema:
            expected_type_str = self.col_to_type_map.get(col_name)
           
            if col_name in X_processed.columns:
                X_for_model[col_name] = X_processed[col_name]
            else:
                ...
                if expected_type_str in ['string', 'object', 'bytes']:
                    ...
                    X_for_model[col_name] = np.nan
                print(f"Warning: Column '{col_name}' was missing and defaulted.")
        ...
        for col_name in self.input_cols_from_schema:
            ...
            try:
                if expected_type_str == 'string':
                    if not pd.api.types.is_string_dtype(actual_dtype):
                        X_for_model[col_name] = X_for_model[col_name].astype(str)
                elif expected_type_str in ['double', 'float']:
                    if not pd.api.types.is_float_dtype(actual_dtype):
                        X_for_model[col_name] = X_for_model[col_name].astype(np.float64)
                elif expected_type_str in ['integer', 'long']:
                    if pd.api.types.is_float_dtype(actual_dtype):
                        if X_for_model[col_name].isnull().any():
                            X_for_model[col_name] = X_for_model[col_name].fillna(0)
                        X_for_model[col_name] = X_for_model[col_name].astype(np.int64)
                ...
                print(f"Error coercing column '{col_name}' to {expected_type_str}. Error: {e}")
        ...
        # Return original data with predictions added
        X_original["days_in_repair"] = predictions
        return X_original
...
# ─── REFACTORED: Capture Benefit Model with Better Preprocessing ─────────────

class CaptureBenefitModel(mlflow.pyfunc.PythonModel):
    """
    Enhanced Capture Benefit Model that ensures proper preprocessing
    ...
    def load_context(self, context):
        ...
        quantile_table_path = context.artifacts["quantile_benefit_table_csv_path"]
        print(f"Loading quantile benefit table from: {quantile_table_path}")
        self.quantile_benefit_df = pd.read_csv(quantile_table_path)

        self.quantile_threshold_df = None
        quantile_threshold_table_path = context.artifacts.get("quantile_threshold_table_csv_path")
       
        if quantile_threshold_table_path and os.path.exists(quantile_threshold_table_path):
            print(f"Loading quantile threshold table from: {quantile_threshold_table_path}")
            self.quantile_threshold_df = pd.read_csv(quantile_threshold_table_path)
            self.quantile_threshold_df = self.quantile_threshold_df.sort_values(by='quantile_threshold').reset_index(drop=True)
            print("Quantile threshold table loaded.")
        else:
            print("Warning: Quantile threshold table not found. Using default categorization.")
    ...
    def predict(self, context, model_input: pd.DataFrame) -> pd.DataFrame:
        ...
        # Transform data
        print("Preprocessing data for capture scenarios...")
        try:
            processed_cap_features = self.preprocessor.transform(df_cap)
            processed_nocap_features = self.preprocessor.transform(df_nocap)
        except Exception as e:
            ...
            print(f"Available columns in input: {list(model_input.columns)}")
        ...
        # Calculate benefits
        raw_capture_benefit = tppd_pred_capture - tppd_pred_no_capture
        print(f"Raw capture benefit - Min: {np.min(raw_capture_benefit)}, Max: {np.max(raw_capture_benefit)}")

        # Create results
        results_df = pd.DataFrame(index=model_input.index)
        results_df['Capture_Benefit_Adjusted'] = raw_capture_benefit * pd.Series(raw_capture_benefit).apply(self._get_quantile_benefit_multiplier).values
        results_df['Capture_Benefit_Priority'] = results_df['Capture_Benefit_Adjusted'].apply(self._get_benefit_priority_category)
...
# ─── Example: Creating Full Pipeline ─────────────────────────────────────────

def create_full_preprocessing_pipeline(categorical_features=None, numerical_features=None):
    """
    Creates a complete preprocessing pipeline that includes all transformations
    needed before the DaysInRepairPredictor.
    """
   
    # Define categorical features if not provided
    if categorical_features is None:
        categorical_features = [
            'incident_cause_description', 'incident_sub_cause_description',
            'postcode_area', 'insurer_name', 'notification_method',
            'incident_uk_country', 'road_conditions', 'weather_conditions'
        ]
    ...
    preprocessing_steps = [
        ...
        ('airbag_count', AirbagCountTransformer()),
        ('car_characteristics', CarCharacteristicTransformer()),
        ('date_features', DateFeatureTransformer()),
        ('vehicle_age', VehicleAgeTransformer()),
        ('rhd_transform', RHDTransformer()),
        ('body_key_encoder', BodyKeyEncoder()),
        ('postcode_extractor', PostcodeAreaExtractor()),
        ('damage_transform', DamageTransformer()),
        ('damage_severity', DamageSeverityCalculator()),
    ]
    ...
    # This would be added after the main preprocessing pipeline
    # to handle categorical encoding
...
# When using the DaysInRepairPredictor, ensure it has access to preprocessed data:
#
# # Option 1: Use the enhanced predictor with built-in preprocessing
# ttr_predictor = DaysInRepairPredictorWithPreprocessing(
#     model_uri="path/to/ttr/model",
#     preprocessor_pipeline=create_full_preprocessing_pipeline()
# )
#
# # Option 2: Ensure data is preprocessed before passing to original predictor
# preprocessor = create_full_preprocessing_pipeline()
# X_preprocessed = preprocessor.fit_transform(X)
# ttr_predictor = DaysInRepairPredictor(model_uri="path/to/ttr/model")
# X_with_ttr = ttr_predictor.transform(X_preprocessed)
...
if __name__ == '__main__':
    # 1. Create a comprehensive sample DataFrame
    print("--- Initial DataFrame Columns ---")
    print(list(df_tpc.columns))
    print("-" * 50)

    # 2. List all transformer classes to be tested in pipeline order
    # Note: We do not include the final ColumnTransformer as its behavior is standard
    transformers_to_test = [
        ('SplatmapExtractor', SplatmapExtractor()),
        ('RenameColumnsTransformer', RenameColumnsTransformer()),
        ('StringNoneToNaNConverter', StringNoneToNaNConverter()),
        ('ImpactSpeedTransformer', ImpactSpeedTransformer()),
        ('AirbagCountTransformer', AirbagCountTransformer()),
        ('CarCharacteristicTransformer', CarCharacteristicTransformer()),
        ('DateFeatureTransformer', DateFeatureTransformer()),
        ('VehicleAgeTransformer', VehicleAgeTransformer()),
        ('RHDTransformer', RHDTransformer()),
        ('BodyKeyEncoder', BodyKeyEncoder()),
        ('PostcodeAreaExtractor', PostcodeAreaExtractor()),
        ('DamageTransformer', DamageTransformer()),
        ('DamageSeverityCalculator', DamageSeverityCalculator()),
        ('GeneralPurposeImputer', GeneralPurposeImputer(
            numerical_features=['fp_vehicle_value', 'tp_mileage'], # Example features
            categorical_features=['incident_cause_description', 'road_conditions'] # Example features
        )),
    ...
    # 3. Sequentially apply each transformer and print the output columns
    current_df = df_tpc.copy()
    for name, transformer in transformers_to_test:
        print(f"--- Applying: {name} ---")
        try:
            # Fit and transform the current state of the DataFrame
            transformer.fit(current_df)
            current_df = transformer.transform(current_df)
            print(f"Output Columns ({len(current_df.columns)}):")
            print(list(current_df.columns))
            display(current_df.head())
        except Exception as e:
            print(f"ERROR processing {name}: {e}")
            print("Columns available at point of failure:")
            print(list(current_df.columns))
            break # Stop on error
        print("-" * 50)
...
numerical_features.remove('days_in_repair')
...
# Instantiate the new, full preprocessing pipeline
# This pipeline now handles all scaling, encoding, imputation, and other transformations.
ttr_preprocessor = create_full_preprocessing_pipeline()

# Fit the preprocessor and transform the raw features
# The output is a pandas DataFrame with consistent column names.
X_processed = ttr_preprocessor.fit_transform(X_raw_features)

# The list of feature names is now derived from the columns of the processed DataFrame
ttr_features_list = X_processed.columns.tolist()

# Now X is a DataFrame, and y is a Series
X = X_processed
y = y_series
...
# Regressor instance (remains the same)
...
# Hyperparameter search (remains the same)
...
# MLflow (remains largely the same)
client = MlflowClient()
...
with mlflow.start_run(run_name="time_to_repair_lgbm_direct_training") as run:
    # We now log the fitted preprocessor as a separate artifact.
    # This allows the `DaysInRepairPredictor` to load it.
    mlflow.sklearn.log_model(ttr_preprocessor, "ttr_preprocessor")
    ...
    # Log the trained model. The signature correctly reflects the preprocessed inputs.
...
num_std = ['fp_deployed_airbags', 'fp_number_of_doors', 'fp_engine_capacity', 'fp_number_of_seats', 'fp_vehicle_value', 'tp_deployed_airbags', 'tp_number_of_doors', 'tp_engine_capacity', 'tp_lights_damaged', 'tp_mileage', 'year_of_manufacture', 'fp_front_severity', 'tp_front_severity', 'fp_front_bonnet_severity', 'tp_front_bonnet_severity', 'fp_front_left_severity', 'tp_front_left_severity', 'fp_front_right_severity', 'tp_front_right_severity', 'fp_left_severity', 'tp_left_severity', 'fp_left_back_seat_severity', 'tp_left_back_seat_severity', 'fp_left_front_wheel_severity', 'tp_left_front_wheel_severity', 'fp_left_mirror_severity', 'tp_left_mirror_severity', 'fp_left_rear_wheel_severity', 'tp_left_rear_wheel_severity', 'fp_left_roof_severity', 'tp_left_roof_severity', 'fp_left_underside_severity', 'tp_left_underside_severity', 'fp_load_area_severity', 'tp_load_area_severity', 'fp_rear_severity', 'tp_rear_severity', 'fp_rear_left_severity', 'tp_rear_left_severity', 'fp_rear_right_severity', 'tp_rear_right_severity', 'fp_rear_window_damage_severity', 'tp_rear_window_damage_severity', 'fp_right_severity', 'tp_right_severity', 'fp_right_back_seat_severity', 'tp_right_back_seat_severity', 'fp_right_front_wheel_severity', 'tp_right_front_wheel_severity', 'fp_right_mirror_severity', 'tp_right_mirror_severity', 'fp_right_rear_wheel_severity', 'tp_right_rear_wheel_severity', 'fp_right_roof_severity', 'tp_right_roof_severity', 'fp_right_underside_severity', 'tp_right_underside_severity', 'fp_roof_damage_severity', 'tp_roof_damage_severity', 'fp_underbody_damage_severity', 'tp_underbody_damage_severity', 'fp_windscreen_damage_severity', 'tp_windscreen_damage_severity', 'impact_speed', 'time_to_notify', 'notified_day_of_week', 'notified_day', 'notified_month', 'notified_year', 'incident_day_of_week', 'incident_day', 'fp_vehicle_age', 'tp_body_key_04', 'tp_damage_assessed', 'damage_sev_total', 'damage_sev_count', 'damage_sev_mean', 'damage_sev_max']
...
# Assume the custom transformers like SplatmapExtractor, RenameColumnsTransformer, etc.,
# and the CaptureBenefitModel class are defined elsewhere in the project as provided previously.

# --- PLACEHOLDERS: User must define these variables ---

# URI for the pre-trained TTR model from the previous MLflow run
ttr_model_uri = "models:/your_ttr_model_name/latest" # Example: "runs:/<run_id>/model"

# Lists of column names for imputation and final transformation
# These must be defined based on the features available in your TPC dataset
num_impute = ['fp_vehicle_value', 'fp_engine_capacity', 'tp_mileage'] # Example list
cat_impute = ['incident_cause_description', 'road_conditions'] # Example list

# TPC-specific feature lists for the final ColumnTransformer
# These should be the column names *after* the main_preprocessor has run
cat_ohe_tpc = ['incident_uk_country', 'postcode_area', 'weather_conditions'] # Example list
num_std_tpc = ['fp_vehicle_value', 'tp_mileage', 'days_in_repair'] # Example list, 'days_in_repair' is generated

# Feature lists for initial data loading
features_list = ['fp_vehicle_value', 'tp_mileage', 'incident_uk_country', 'postcode_area', 'weather_conditions', 'incident_cause_description', 'road_conditions'] # Example list
date_columns = ['incident_date', 'notification_date'] # Example list
tpc_target_column = 'inc_tot_tppd'

# MLflow model registration name
tpc_model_name = "third_party_capture_model"

# --- PIPELINE DEFINITION ---

# Step 2: The main preprocessor for the final TPC model
main_preprocessor = Pipeline([
    ('splatmap_extractor', SplatmapExtractor()),
    ('rename_columns', RenameColumnsTransformer()),
...
# The final combined pipeline for the TPC model
full_tpc_pipeline = Pipeline([
    # Step 1: Generate 'days_in_repair' feature using the pre-trained TTR model and its preprocessor
    ('days_in_repair_feature_generator', DaysInRepairPredictor(model_uri=ttr_model_uri)),

    # Step 2: Apply main feature engineering steps for the TPC model
    ("main_preprocessor", main_preprocessor),

    # Step 3: Final scaling and encoding for the TPC model features
    ("final_col_transformer", ColumnTransformer(transformers=[
        ("cat", OneHotEncoder(handle_unknown="ignore", sparse_output=False), cat_ohe_tpc),
        ("num", StandardScaler(), num_std_tpc),
    ], remainder="passthrough",
    verbose_feature_names_out=False).set_output(transform="pandas")),

    # Step 4: The final regressor for TPC prediction
...
# --- LOAD & SPLIT DATA ---
# This part assumes a DataFrame `df_tpc` is already loaded
# df_tpc = spark.read.table("your_source_table").toPandas()
# df_tpc = standardize_pandas_schema(df_tpc) # Assuming a standardization function exists

df_tpc = df_tpc[features_list + date_columns + [tpc_target_column]]
df_tpc[tpc_target_column] = df_tpc[tpc_target_column].fillna(0)
df_tpc = df_tpc[(df_tpc[tpc_target_column] > 0) & (df_tpc[tpc_target_column] < 20000)]
X = df_tpc.drop(tpc_target_column, axis=1)
y = df_tpc[tpc_target_column]
...
# Define MLflow Schemas based on raw input and final output
raw_input_cols = [ColSpec(type=str(X_train[col].dtype), name=col) for col in X_train.columns]
input_schema = Schema(raw_input_cols)
output_schema = Schema([ColSpec(type="double", name="Capture_Benefit_Adjusted")])


with mlflow.start_run(run_name="third_party_capture_with_benefit_logic") as run:
    print(f"MLflow Run ID: {run.info.run_id}")

    # The hyperparameter grid targets steps in the final pipeline
    param_distributions = {
        "regressor__n_estimators": [350, 400, 450, 500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000],
        "regressor__max_depth": [2, 3, 4, 5],
        "regressor__min_child_weight": uniform(loc=0, scale=0.2),
        "regressor__subsample": [0.8, 0.9, 1.0],
    }

    # RandomizedSearchCV uses the complete TPC pipeline
    random_search = RandomizedSearchCV(
        full_tpc_pipeline,
    ...
    # Create a temporary pipeline of all preprocessing steps to get final feature names
    preprocessor_for_names = Pipeline(best_overall_model.steps[:-1])
    X_train_transformed_for_names = preprocessor_for_names.transform(X_train.head(10))
    all_feat_names_transformed = X_train_transformed_for_names.columns.tolist()

    monotonicity_dict = {
        "capturesuccess_adrienneversion": -1 # Name should be checked against `all_feat_names_transformed`
    }

    monotonicity_list = []
    for feat_name in all_feat_names_transformed:
        base_feat_name = feat_name.split('__')[-1]
    ...
    # Extract the fitted components for the PyFunc model
    fitted_preprocessor_component = Pipeline(best_overall_model.steps[:-1])
    ...
    best_params['monotone_constraints'] = str(monotonicity_list)
    ...
    # Save the fitted components locally to be included as artifacts
    ...
    # Save lookup tables locally to be included as artifacts
    ...
    # Define artifacts dictionary for the custom PyFunc model
    ...
    shutil.rmtree("pipeline_artifacts")
